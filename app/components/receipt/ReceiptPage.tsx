...
const ReceiptPage: React.FC<CompletePageProps> = ({ bookingId }) => {
  ...
  const [showPdfTemplate, setShowPdfTemplate] = useState<boolean>(false);

  const generatePdf = async () => {
    // First show the PDF template to make it renderable
    ...
    if (input) {
      try {
        const canvas = await html2canvas(input, {
          ...
          windowWidth: 210 * 3.78, // A5 landscape width in pixels at 96 DPI
          windowHeight: 155 * 3.78, // Increased from 152 to 155 to capture more at the bottom
          x: 0,
          y: 0,
          width: 210 * 3.78,
          height: 155 * 3.78, // Increased from 152 to 155 to match windowHeight
        });

        const imgData = canvas.toDataURL('image/png', 1.0);

        // Create a landscape PDF with A5 dimensions
        const pdf = new jsPDF({
          orientation: 'landscape',
          unit: 'mm',
          format: 'a5', // Standard A5 size
        });
        ...
        // Add metadata
        pdf.setProperties({
          title: `USE Receipt - ${users?.name}`,
          subject: `Transaction Receipt for ${bookingSchedule?.passid}`,
          creator: 'Union of Security Employees',
          keywords: 'receipt, payment, security',
        });
    ...
    const fetchBookingSchedule = async () => {
      try {
        ...
        const responseBookingSchedule = await fetch(
          `/api/get-booking-schedule?bookingId=${encodeURIComponent(bookingId)}`
        );
        if (
          !responseBookingSchedule.ok &&
          responseBookingSchedule.status === 401
        ) {
          setLoading(false);
          router.push('/signin');
        ...
        const dataBookingSchedule: booking_schedules =
          await responseBookingSchedule.json();
    ...
    <form>
      ...
      <div className={receiptContentstyles.container}>
        <div className={receiptContentstyles.innerContainerHeader}>
          ...
          <div className={globalStyleCss.header1}>Payment receipt</div>
        ...
        {/* PDF template - hidden by default but displayed right before PDF generation */}
        <div
          ref={pdfRef}
          style={{
            position: showPdfTemplate ? 'absolute' : 'fixed',
            left: showPdfTemplate ? '0' : '-9999px',
            top: showPdfTemplate ? '0' : '-9999px',
            width: showPdfTemplate ? '210mm' : 'auto', // A5 landscape width
            height: showPdfTemplate ? '155mm' : 'auto', // Increased from 148mm to 155mm
            backgroundColor: 'white',
            zIndex: showPdfTemplate ? '-1' : '-9999', // Behind content but still renderable
            overflow: 'hidden',
            opacity: showPdfTemplate ? '1' : '0',
          }}
        >
          <ReceiptPDF
            users={users}
        ...
        <div className={receiptContentstyles.innerContainer}>
          <div className={receiptContentstyles.receiptTextBox1}>
            <div className={receiptContentstyles.boxWidth}>
              <div className={globalStyleCss.header2}>Transaction details</div>
            </div>
          </div>
          <div className={receiptContentstyles.receiptTextBox1}>
            <div className={receiptContentstyles.boxWidth}>
          ...
          <div className={receiptContentstyles.receiptTextBox1}>
            ...
            <div className={receiptContentstyles.boxWidth}>
              ...
              >
                {' '}
                {appTypeMap[bookingSchedule?.app_type || ''] ||
                  'Unknown'} -{' '}
              ...
              >
                ...
                >
                  <g clipPath='url(#clip0_1433_2507)'>
                    <path
                      d='M12 2C6.48 2 2 6.48 2 12C2 17.52 6.48 22 12 22C17.52 22 22 17.52 22 12C22 6.48 17.52 2 12 2ZM13 16C13 16.55 12.55 17 12 17C11.45 17 11 16.55 11 16V12C11 11.45 11.45 11 12 11C12.55 11 13 11.45 13 12V16ZM12 9C11.45 9 11 8.55 11 8C11 7.45 11.45 7 12 7C12.55 7 13 7.45 13 8C13 8.55 12.55 9 12 9Z'
                      fill='#0277BD'
                    />
                  </g>
                  <defs>
                ...
                <div className={globalStyleCss.regular}>
                  &nbsp;Please note that the appointment details shown here are
                  from your original booking and will not be updated on this
                  receipt should you reschedule.
                </div>
              </div>
            </div>
          </div>
